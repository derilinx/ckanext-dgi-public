...
from ckanext.dgu.authorize import (
                             ...
                             dgu_group_change_state, dgu_group_update, dgu_group_user_update
                             )
...
class ThemePlugin(p.SingletonPlugin):
    ...
    def update_config(self, config):
        ...
        if not config.get('dgu.shared_assets_timestamp_path'):
            toolkit.add_public_directory(config, '../../../shared_dgi_assets')
    ...
    def before_map(self, map):
        ...
        #seems to point to data no matter what
        map.connect('/data', controller=theme_controller, action='index')
        map.connect('/home', controller=theme_controller, action='index')
        map.redirect('/', '/home')
...
class AuthApiPlugin(p.SingletonPlugin):
    ...
    def get_auth_functions(self):
        return {
                ...
                'group_user_update': dgu_group_user_update,
                'organization_update': dgu_group_update,
                'organization_user_update': dgu_group_user_update,
...
class DguForm(p.SingletonPlugin):
    ...
    def before_map(self, map):
        ...
        #map.connect('dataset_suggest', '/dataset/suggest', controller=dgu_package_controller, action='suggest')
        map.connect('dataset_suggest_issue', '/dataset/suggest-issue', controller=dgu_package_controller, action='suggest_issue')
...
class SearchPlugin(p.SingletonPlugin):
    ...
    def before_search(self, search_params):
        ...
        # within.
        search_params['qf'] = 'title^4 name^3 notes^2 agency^2 text tags^0.3 group_titles^0.3 extras_harvest_document_content^0.2'
        ...
        if order_by in (None, 'spatial desc') and sort_by_location_enabled:
            ...
            search_params['sort'] = 'score desc, popularity desc, metadata_created desc'
    ...
    def before_index(self, pkg_dict):
        ...
        SearchIndexing.add_field__license_id(pkg_dict)
        ...
        SearchIndexing.add_linked_data_flag(pkg_dict)
