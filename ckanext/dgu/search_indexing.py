...
class SearchIndexing(object):
    ...
    @classmethod
    def add_field__license_id(cls, pkg_dict):
        '''Adds the license_id field.'''
        if 'license_id' not in pkg_dict:
            pkg_dict['license_id'] = 'other-at'
    ...
    @classmethod
    def add_linked_data_flag(cls, pkg_dict):
        '''Check if any resource is 5 stars and set flag accordingly'''
        pkg_dict['linked_data'] = False

        if 'res_format' in pkg_dict:
            five_start_formats = Formats.get_x_stars_formats(5)
            is_five_star = [True for p in pkg_dict['res_format'] if p.upper() in five_start_formats]
            if is_five_star:
                pkg_dict['linked_data'] = True

            log.debug('Linked Data: %s', pkg_dict['linked_data'])
