...
from ckanext.dgu.schema import LICENSES
...
def organization_list_with_datasets():
    from ckan import model
    organizations = model.Session.query(model.Group).\
        filter(model.Group.type=='organization').\
        filter(model.Group.state=='active').order_by('title')
    ...
    if is_sysadmin():
        ...
        for organization in organizations:
            q = '''select package.id from package where package.owner_org = '%s' and package.state = 'active' and package.type = 'dataset' ''' % organization.id
            row = model.Session.execute(q).fetchone()

            if row:
                yield organization

def organization_list_no_blanks():
    from ckan import model
    ...
    if is_sysadmin():
        ...
        out = [org for org in organizations if len(org.packages()) > 0]
    return out
...
def get_wms_info(pkg_dict, returnEmptyString=False, emptyValue=''):
    ...
    * extent is a tuple of (N, W, E, S) representing max extent
    ...
    if len(extent_list) == 0 and returnEmptyString:
        return emptyValue
    else:
...
def get_primary_theme(package, fullName=False):
    primary_theme = get_from_flat_dict(package.get('extras', []), 'theme-primary', '')
    if fullName:
...
def get_tag_list (package, toString=False):
    lst = []
    for d in package.get('tags', []):
        tag = d.get('name')
        if tag:
            lst.append(tag)
    if toString:
...
def get_geographic_bbox (pkg_extras, noValue=None):
    extras_dict = dict(pkg_extras)
    if 'bbox-east' not in extras_dict and 'bbox-west' not in extras_dict and 'bbox-north' not in extras_dict and 'bbox-south' not in extras_dict:
        return noValue
    bb_east = extras_dict.get('bbox-east', 'n/a')
    bb_west = extras_dict.get('bbox-west', 'n/a')
    bb_north = extras_dict.get('bbox-north', 'n/a')
    bb_south = extras_dict.get('bbox-south', 'n/a')
    return ''.join(['North: ', bb_north, ', South: ', bb_south, ', East: ', bb_east, ', West: ', bb_west])

def get_geographic_coverage (pkg_extras, noValue=None):
    extras_dict = dict(pkg_extras)
    if 'geographic_coverage' in extras_dict:
        return extras_dict.get('geographic_coverage')
    elif 'geographic_coverage-other' in extras_dict:
        return extras_dict.get('geographic_coverage-other')
    else:
        return get_geographic_bbox(pkg_extras, noValue)

def get_vertical_extent (pkg_extras, noValue=None):
    extras_dict = dict(pkg_extras)
    if 'vertical_extent' not in extras_dict:
        return noValue
    try:
        data = json.loads(extras_dict.get('vertical_extent'))
        nameVal = data.get('verticalDomainName', 'n/a')
        minVal = data.get('minVerticalExtent', 'n/a')
        maxVal = data.get('maxVerticalExtent', 'n/a')
        # saved for posterity
        #return ''.join(['Domain name: ', nameVal, ', Min value: ', minVal, ', Max value: ', maxVal])
        return "Domain name: %s, Min value: %s, Max value: %s" % (nameVal, minVal, maxVal)
    except ValueError:
        #bad json string
        vertical_extent = extras_dict.get('vertical_extent')
        if vertical_extent == '':
            return noValue
        else:
            return vertical_extent

def get_temporal_coverage (pkg_extras, noValue=None):
    extras_dict = dict(pkg_extras)
    if 'temporal_coverage-from' not in extras_dict and 'temporal_coverage-to' not in extras_dict:
        return noValue
    temp_from = extras_dict.get('temporal_coverage-from', 'n/a')
    temp_to = extras_dict.get('temporal_coverage-to', 'n/a')
    return ''.join(['From: ', temp_from, ', To: ', temp_to])
...
def predict_if_resource_will_preview(resource_dict):
    ...
    if normalised_format in ('csv', 'tsv'):
        ...
        return file_extension and file_extension in ('csv', 'tsv')
    else:
...
def resource_to_preview_on_dataset_page(resources):
    ...
    preview_priority = [
        # if we have a kml resource we want to show that first
        'kml',
...
def is_preview_geo_resource(resource):
    normalised_format = resource.get('format').lower().split('/')[-1]
    if normalised_format in (('geojson', 'kml')):
        return resource
...
def star_caption(num_stars):
    captions = [
        'Unavailable, not openly licensed, or as yet uncalculated',
        'Unstructured data (e.g. PDF)',
        'Structured data but proprietary format (e.g. Excel)',
        'Structured data in open format (e.g. CSV)',
        'Linkable data - served at URIs (e.g. RDF)',
...
def get_package_sparql_endpoint(resources):
    for res in resources:
        if res['format'].upper() == 'SPARQL' and res['url']:
            return res['url']
    return None


def get_package_open_standards_raw(id):
    open_standards = []
    if is_plugin_enabled('openstandard'):
        from ckan import model

        relationships = model.PackageRelationship.by_subject(model.Package.get(id))
        for rel in relationships:
            open_standards.append(rel.as_dict()['subject'])

    return open_standards


def get_package_open_standards(id):
    open_standards = []
    if is_plugin_enabled('openstandard'):
        relation_type = 'links_to'

        context = {'model': ckan.model, 'session': ckan.model.Session, 'for_view': True}
        data_dict = {'id': id, 'use_default_schema': True}

        pkg_dict = ckan.logic.get_action('package_show')(context, data_dict)
        # somehow get the open standard associated with the given dataset
        open_standards = []
        for rel in pkg_dict['relationships_as_subject']:
            if rel['type'] == relation_type and rel['state'] == 'active':
                open_standard_id = rel.get('object_package_id')
                open_standards.append(ckan.logic.get_action('package_show')(context, {"id": open_standard_id}))

    return open_standards


def get_open_standards_dicts():
    '''Return a list of datasets of type openstandard '''
    open_standards = []
    if is_plugin_enabled('openstandard'):
        from ckan import model
        standards = model.Session.query(model.Package) \
                .filter(model.Package.type == 'openstandard') \
                .filter(model.Package.state == 'active').all()

        for open in standards:
            open_standards.append(open.as_dict())

    return open_standards

def get_selected_open_standards_for_package(pkg_id):
    '''Return a list of open standard ids that are linked_from the pkg_id dataset'''
    return [open_standard['name'] for open_standard in get_package_open_standards(pkg_id)]
...
def dgu_drill_down_url(params_to_keep, added_params, package_type='dataset'):
...
def get_resource_fields(resource, pkg_extras):
    ...
    field_value_map = {
        ...
        'date-updated-computed': {'label': 'Date updated', 'label_title': 'Date updated on data.localgov.ie', 'value': render_datestamp(res_dict.get('revision_timestamp'))},
...
def get_package_fields(package, pkg_extras, dataset_was_harvested,
    ...
    field_value_map = {
        # field_name : {display info}
        'date_added_to_dgu': {'label': 'Added to data.localgov.ie', 'value': package.metadata_created.strftime('%d/%m/%Y')},
        'date_updated_on_dgu': {'label': 'Updated on data.localgov.ie', 'value': package.metadata_modified.strftime('%d/%m/%Y')},
...
def get_package_fields_dict(package, pkg_extras, dataset_was_harvested,
                       is_location_data, dataset_is_from_ns_pubhub, is_local_government_data):
    fields = get_package_fields(package, pkg_extras, dataset_was_harvested, is_location_data, dataset_is_from_ns_pubhub, is_local_government_data)
    fields_dict = {}
    for field_tup in fields:
        name = field_tup[0].get('name')
        val = field_tup[0].get('value')
        fields_dict[name] = val
    return fields_dict

def results_sort_by():
    # Default to location if there is a bbox and no other parameters. Otherwise
    # relevancy if there is a keyword, otherwise last updated.
    ...
    default_search = 'metadata_created'
    return c.sort_by_fields or ('spatial' if not sort_by_location_disabled() else ('rank' if c.q else default_search))
...
def dataset_count_for_publisher(orgid):
    from ckan import model
    return model.Session.query(model.Package)\
        .filter(model.Package.owner_org==orgid)\
        .filter(model.Package.type=='dataset')\
        .filter(model.Package.state=='active').count()

def dataset_count_for_publisher_under_cc(orgid):
    from ckan import model
    return model.Session.query(model.Package)\
        .filter(model.Package.owner_org==orgid)\
        .filter(model.Package.type=='dataset')\
        .filter(model.Package.license_id=='cc-by')\
        .filter(model.Package.state=='active').count()
...
def source_count():
    ...
    count = 0
    if is_sysadmin():
        count = len([org for org in orgs])
    else:
        for org in orgs:
            has_dataset = model.Session.query(model.Package)\
...
def license_choices(data):
    license_ids = LICENSES.keys()
    selected_license = data.get('license_id')
    return [(id, LICENSES[id]) for id in license_ids]
...
def get_date_released (data):
    date_released = data.get('date_released', '')
    if isinstance(date_released, list):
        date_released = date_released[0]
...
def search_facets_unselected(facet_keys,sort_by='count',package_type='dataset'):
    unselected_raw = []
    for key in facet_keys:
        if 'county' in facet_keys and len(facet_keys) is 1:
            limit = 50
        else:
            limit = 10
        for value in unselected_facet_items(key, limit=limit):
...
def search_facets_selected(facet_keys, package_type='dataset'):
    selected = []
...
def search_facet_text(key,value):
    ...
    if key=='openness_score':
        ...
        if stars == -1:
            return 'TBC'
        return t.literal( (stars * '<i class="icon-star"></i>') + ('<i class="icon-star-empty"></i>' * (5-stars)) )
    if key == 'api':
        if value == 'true':
    ...
    if key == 'linked_data':
        if value == 'true':
            return 'Show Linked Data datasets'
        return 'Hide Linked Data datasets'
    ...
    if key == 'license_id':
        return LICENSES.get(value, value)
    if key == 'res_format':
        return value.upper()
...
def has_resources_linked_data(resources):
    formats = [r.get('format', '').upper() for r in resources]
    five_start_formats = Formats.get_x_stars_formats(5)
    is_five_star = [True for f in formats if f in five_start_formats]
    return is_five_star  # Empty list which is false, or non-empty list which means true


def has_package_linked_data(pkg_dict):
    return has_resources_linked_data(pkg_dict.get('resources', []))


def has_resources_api(resources):
    for r in resources:
        log.error(r.get('format', '').lower())
    return 'api' in [r.get('format', '').lower() for r in resources]


def has_package_api(pkg_dict):
    return has_resources_api(pkg_dict.get('resources', []))


def get_api_formats_list(api_response_formats):
    log.error([x.strip() for x in api_response_formats.split(',')])
    return [x.strip() for x in api_response_formats.split(',')]
...
def get_shared_assets_timestamp():
    ...
    if shared_assets_timestamp is None:
        ...
        if not timestamp_filepath:
            # Default place to find shared_dgi_assets repo is next to this
            # repo - perfect for developers
            import os
            this_file = os.path.dirname(os.path.realpath(__file__))
            timestamp_filepath = os.path.join(this_file, '..', '..', '..', '..', 'shared_dgi_assets', 'assets', 'timestamp')
        try:
...
def is_unpublished_item(package):
    ...
    if package.get('type') is 'harvest':
        return True
...
def get_user_drupal_roles(user):
    ...
    roles = []
    drupal_id = DrupalUserMapping.ckan_user_name_to_drupal_id(user)
    try:
        if drupal_id:
            drupal_client = DrupalClient()
            user_properties = drupal_client.get_user_properties(drupal_id)
