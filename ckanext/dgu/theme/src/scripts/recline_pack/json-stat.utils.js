	...
	function tbrowser(jsonstat, selector, options){
		...
		function setup(ds, preset){
			...
			if(preset.type){
				...
				rows=arr[0].id;
				cols=arr[1].id;
                        } else if (preset.rows && preset.cols) {
                                rows=preset.rows;
                                cols=preset.cols;
		...
		function HTMLtable(element, ds, o, tblclass){
			...
			//Caption
			caption+="<caption><div class='row'><div class='col-md-12'>"+title+"</div></div><div class='row'><div class='col-md-12'><form>";
			...
			for(var name in filter){
				...
				if(dim.length>1){
					filtfield+="<p><strong>"+label+": </strong> "+select(ds, name, [filter[name], null])+"</p>";
				}else{
			...
			if(filtfield!==""){
				filtfield='<div class="col-md-6"><fieldset id="filters"><legend>'+msgs.filters+'</legend>'+filtfield+'</fieldset></div>';
			}
			...
			if(constfield!==""){
				constfield='<div class="col-md-6"><fieldset id="constants"><legend>'+msgs.constants+'</legend>'+constfield+'</fieldset></div>';
			}

                        var rowcolsf='<div class="col-md-6"><fieldset id="rowscols"><legend>'+msgs.rc+'</legend>'+select(ds, "rows", [r, c])+' <a>&#x2194;</a> '+select(ds, "cols", [c, r])+"</fieldset></div>";

			caption+=rowcolsf+constfield+filtfield;
                        caption += "</form></div></div>" + "<div class='row'><div class='col-md-12' id='tbrowser-chart' style='height:400px;'></div></div></caption>"
			...
			//Head
			head+="<thead><tr><th style='text-align: left'>" + r + "</th>";
				...
				e.addEventListener("change", function(event) {
					...
					HTMLtable(
						...
						tblclass
                                        ...
                                        if (options.onchange) {options.onchange.call()}
				}, false);
